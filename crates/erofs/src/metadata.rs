...
mod tests {
    ...
    #[test]
    fn applies_android_ownership_modes_capabilities_and_contexts() {
        let temp = tempfile::tempdir().unwrap();
        let file = temp.path().join("app+one");
        fs::write(&file, b"test").unwrap();
        let config = temp.path().join("fs_config");
        let contexts = temp.path().join("file_contexts");
        fs::write(
            &config,
            "system/app+one 123456 456789 6751 capabilities=0x123456789abcdef\n",
        )
        .unwrap();
        fs::write(
            &contexts,
            "/system/app\\+one -- u:object_r:system_file:s0\n",
        )
        .unwrap();
        let options = BuildOptions {
            fs_config: Some(config),
            file_contexts: Some(contexts),
            mount_point: "system".into(),
            ..Default::default()
        };
        let resolved = MetadataResolver::load(&options)
            .unwrap()
            .resolve("/app+one", &file, &fs::symlink_metadata(&file).unwrap())
            .unwrap();
        assert_eq!(
            (resolved.uid, resolved.gid, resolved.mode & 0o7777),
            (123456, 456789, 0o6751)
        );
        assert_eq!(
            capability_bits(&resolved.xattrs["security.capability"]),
    ...
    #[test]
    fn ownership_offsets_are_checked() {
        assert!(adjusted_id(0, -1).is_err());
        assert!(adjusted_id(u32::MAX, 1).is_err());
        assert_eq!(adjusted_id(1000, -500).unwrap(), 500);
    }

    #[test]
    fn unchanged_selinux_context_preserves_original_binary_terminator() {
        let temp = tempfile::tempdir().unwrap();
        ...
        for label in [
            ...
            let mut entry = host_entry(&fs::symlink_metadata(&file).unwrap()).unwrap();
            entry
                .xattrs
                .insert("security.selinux".into(), label.clone());
            let metadata = MetadataManifest {
                ...
                build_time: 0,
                extraction_root: None,
                entries: BTreeMap::from([("/file".into(), entry)]),
            };
            fs::write(&metadata_file, serde_json::to_vec(&metadata).unwrap()).unwrap();
            let options = BuildOptions {
                metadata_file: Some(metadata_file.clone()),
                file_contexts: Some(contexts.clone()),
                ..Default::default()
            };
            let actual = MetadataResolver::load(&options)
                .unwrap()
                .resolve("/file", &file, &fs::symlink_metadata(&file).unwrap())
                .unwrap();
            assert_eq!(actual.xattrs["security.selinux"], label);
        }
    }

    #[cfg(windows)]
    #[test]
    fn windows_symlinks_preserve_moved_originals_and_accept_edited_targets() {
        ...
        let original = EntryMetadata {
            ...
            rdev: 0,
            original_nid: Some(9),
            symlink: Some("/system/bin/tool".to_owned()),
            xattrs: BTreeMap::new(),
        };
        let resolver = MetadataResolver {
            ...
            config: BTreeMap::new(),
            contexts: Vec::new(),
            manifest: Some(MetadataManifest {
                version: 1,
                block_size: 4096,
                uuid: [0; 16],
                volume_label: String::new(),
                build_time: 0,
                extraction_root: Some(fs::canonicalize(&source).unwrap().join("unused").join("..")),
                entries: BTreeMap::from([("/link".to_owned(), original)]),
            }),
        };
        symlink_file(source.join("system/bin/tool"), source.join("link")).unwrap();
        let resolve = |root: &Path| {
            resolver.resolve(
                "/link",
                &root.join("link"),
                &fs::symlink_metadata(root.join("link")).unwrap(),
            )
        };
        assert_eq!(
            resolve(&source).unwrap().symlink.as_deref(),
            Some("/system/bin/tool")
        );
        let moved = temporary.path().join("moved");
        fs::rename(&source, &moved).unwrap();
        assert_eq!(
            resolve(&moved).unwrap().symlink.as_deref(),
            Some("/system/bin/tool")
        );
        fs::remove_file(moved.join("link")).unwrap();
        symlink_file(moved.join("system/bin/other"), moved.join("link")).unwrap();
        assert_eq!(
            resolve(&moved).unwrap().symlink.as_deref(),
            Some("/system/bin/other")
        );
        fs::remove_file(moved.join("link")).unwrap();
        symlink_file("../relative", moved.join("link")).unwrap();
        assert_eq!(
            resolve(&moved).unwrap().symlink.as_deref(),
            Some("../relative")
        );
        fs::remove_file(moved.join("link")).unwrap();
        symlink_file(
            temporary.path().join("\u{4e2d}/outside"),
            moved.join("link"),
        )
        .unwrap();
        assert!(resolve(&moved).is_err());
    }
}
