...
mod tests {
    ...
    #[test]
    fn rejects_truncated_image_and_out_of_bounds_plain_data() {
        let temp = tempfile::tempdir().unwrap();
        let source = temp.path().join("source");
        fs::create_dir(&source).unwrap();
        fs::write(source.join("data"), vec![1; 16384]).unwrap();
        ...
        let device = crate::io::Device::open(image.to_str().unwrap(), 0).unwrap();
        let sb = Arc::new(crate::sb::erofs_read_superblock(device).unwrap());
        let inode = crate::dir::erofs_ilookup(&sb, "/data").unwrap();
        let mut file = OpenOptions::new().write(true).open(&image).unwrap();
        file.seek(SeekFrom::Start(inode.iloc() + 16)).unwrap();
        file.write_all(&(sb.primarydevice_blocks as u32).to_le_bytes())
            .unwrap();
        assert!(
            format!("{:#}", verify_image(&image).unwrap_err())
                .contains("data extent extends beyond")
        );
